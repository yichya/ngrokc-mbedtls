...
int SendReqTunnel(int sock, ssl_context* ssl, char* ReqId, TunnelInfo* tunnelInfo) {
    ...
    sprintf(str,
            "{\"Type\":\"ReqTunnel\",\"Payload\":{\"Protocol\":\"%s\",\"ReqId\":\"%s\",\"Hostname\": \"%s\",\"Subdomain\":\"%s\",\"HttpAuth\":\"\",\"RemotePort\":%d,\"authtoken\":\"%s\"}}",
            tunnelInfo->protocol, ReqId, tunnelInfo->hostname, tunnelInfo->subdomain, tunnelInfo->remoteport,
            mainInfo.authtoken);
